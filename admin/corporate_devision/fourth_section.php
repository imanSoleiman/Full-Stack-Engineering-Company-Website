...
// Get section title and paragraph
$sectionData = $conn->query("SELECT * FROM corporate_fourth_section WHERE id = 1")->fetch_assoc();

// Update section info
if (isset($_POST['save_section'])) {
    $title = $_POST['section_title'];
    $paragraph = $_POST['section_paragraph'];
    $stmt = $conn->prepare("UPDATE corporate_fourth_section SET section_title=?, section_paragraph=? WHERE id=1");
...
if (isset($_POST['add_card'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO corporate_fourth_cards (icon_class, card_title, card_text) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $icon, $title, $text);
    $stmt->execute();
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM corporate_fourth_cards WHERE id=$id");
...
if (isset($_POST['update_card'])) {
    ...
    $title = $_POST['card_title'];
    $text = $_POST['card_text'];
    $stmt = $conn->prepare("UPDATE corporate_fourth_cards SET icon_class=?, card_title=?, card_text=? WHERE id=?");
    $stmt->bind_param("sssi", $icon, $title, $text, $id);
    $stmt->execute();
...
<title>Edit Fourth Section - Customer Satisfaction</title>
...
form input[type="text"], form textarea, form select { width: 100%; padding: 12px 15px; margin-bottom: 20px; border: 1px solid #ccc; border-radius: 8px; font-size: 14px; transition: 0.3s; }
form input[type="text"]:focus, form textarea:focus, form select:focus { border-color: #007bff; outline: none; box-shadow: 0 0 5px rgba(0,123,255,0.2); }
...
.card { border: 1px solid #e0e0e0; border-radius: 10px; padding: 15px 20px; margin-bottom: 15px; background-color: #f9fafb; display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; }
.card form { flex-grow: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px; }
...
<div class="container">
<h2>Edit Fourth Section - Customer Satisfaction</h2>
...
<form method="POST">
    ...
    <textarea name="section_paragraph" rows="6" required><?= htmlspecialchars($sectionData['section_paragraph']) ?></textarea>
...
<form method="POST">
    ...
    <select name="icon_class">
        ...
        <option value="fa-solid fa-money-bill">Money Bill</option>
    ...
    <div class="card">
        <form method="POST">
            ...
            <select name="icon_class" required>
                <option value="fa-solid fa-user" <?= $card['icon_class']=='fa-solid fa-user'?'selected':'' ?>>User</option>
                <option value="fa-solid fa-handshake" <?= $card['icon_class']=='fa-solid fa-handshake'?'selected':'' ?>>Handshake</option>
                <option value="fa-solid fa-money-bill" <?= $card['icon_class']=='fa-solid fa-money-bill'?'selected':'' ?>>Money Bill</option>
                <option value="fa-solid fa-heart" <?= $card['icon_class']=='fa-solid fa-heart'?'selected':'' ?>>Heart</option>
                <option value="fa-solid fa-star" <?= $card['icon_class']=='fa-solid fa-star'?'selected':'' ?>>Star</option>
            </select>
            ...
            <a href="?delete=<?= $card['id'] ?>" onclick="return confirm('Are you sure?')" style="color:#ff4d4f;">Delete</a>
        </form>
    </div>
<?php endwhile; ?>
