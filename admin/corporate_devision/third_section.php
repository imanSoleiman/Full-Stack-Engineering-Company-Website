...
// Fetch current data
$result = $conn->query("SELECT * FROM corporate_third_section WHERE id = 1");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("UPDATE corporate_third_section SET header_title=?, paragraph1=?, paragraph2=?, image_path=?, card1_title=?, card1_text=?, card2_title=?, card2_text=? WHERE id=1");
    $stmt->bind_param("ssssssss", $header_title, $paragraph1, $paragraph2, $image_path, $card1_title, $card1_text, $card2_title, $card2_text);
    $stmt->execute();
...
<title>Edit Third Section - Social Responsibility (People)</title>
<style>
    body {
...
<div class="container">
    <h2>Edit Third Section - Social Responsibility (People)</h2>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <input type="text" name="header_title" value="<?= htmlspecialchars($data['header_title']) ?>" required>
